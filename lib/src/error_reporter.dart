    ...
    try {
      ...
      try {
        final response = await HttpsService.post(
            ...
            resType: resType,
            charset: charset,
            retryIf: ErrorReporterConfig().retryCondition,
            maxRetries: ErrorReporterConfig().maxRetries,
            baseDelay: ErrorReporterConfig().baseDelay,
            maxJitter: ErrorReporterConfig().maxJitter);
        ...
        if (_avoidDuplicate && errorHash != null) {
          if (_sentErrorHashes.length >= 1000) {
            _sentErrorHashes.remove(_sentErrorHashes.first);
          }
          _sentErrorHashes.add(errorHash);
        }
      } catch (e) {
        if (kDebugMode) {
          debugPrint('[ErrorReporter] Error reporting failed: $e');
        }
